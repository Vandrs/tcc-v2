...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot(GateContract $gate)
    {
        ...
        $gate->define(EnumCapabilities::MANAGE_PROJECT, function($user, C3Project $project){
            $exists = $project->getMembers()
                              ->where('id', $user->id)
                              ->whereIn('role',[EnumProject::ROLE_OWNER, EnumProject::ROLE_CONTRIBUTOR, EnumProject::ROLE_MENTOR], false)
                              ->first();
            if($exists){
                return true;
            }
        ...
        $gate->define(EnumCapabilities::PROJECT_OWNER, function($user, C3Project $project){
            $exists = $project->getMembers()
                              ->where('id', $user->id)
                              ->whereIn('role',[EnumProject::ROLE_OWNER], false)
                              ->first();
            if($exists){
                return true;
            }
